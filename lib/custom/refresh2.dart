...
// How much the scroll's drag gesture can overshoot the RefreshIndicator2's
// displacement; max displacement = _kDragSizeFactorLimit * displacement.
const double _kDragSizeFactorLimit = 1.5;

// When the scroll ends, the duration of the refresh indicator's animation
// to the RefreshIndicator2's displacement.
...
/// Used to configure how [RefreshIndicator2] can be triggered.
enum RefreshIndicator2TriggerMode {
...
///
/// The trigger mode is configured by [RefreshIndicator2.triggerMode].
///
/// {@tool dartpad}
/// This example shows how [RefreshIndicator2] can be triggered in different ways.
///
/// ** See code in examples/api/lib/material/refresh_indicator/refresh_indicator.0.dart **
/// {@end-tool}
///
/// {@tool dartpad}
/// This example shows how to trigger [RefreshIndicator2] in a nested scroll view using
...
///
/// The [RefreshIndicator2] will appear if its scrollable descendant can be
/// overscrolled, i.e. if the scrollable's content is bigger than its viewport.
/// To ensure that the [RefreshIndicator2] will always appear, even if the
...
class RefreshIndicator2 extends StatefulWidget {
  ...
  /// If it is null, it will be defaulted to [MaterialLocalizations.RefreshIndicator2SemanticLabel].
  ...
  const RefreshIndicator2({
    ...
    this.triggerMode = RefreshIndicator2TriggerMode.onEdge,
  });

  /// Creates an adaptive [RefreshIndicator2] based on whether the target
  ...
  const RefreshIndicator2.adaptive({
    super.key,
    required this.child,
    this.displacement = 40.0,
    this.edgeOffset = 0.0,
  ...
  ///
  /// This will be defaulted to [MaterialLocalizations.RefreshIndicator2SemanticLabel]
  /// if it is null.
  ...
  /// Defines [strokeWidth] for `RefreshIndicator2`.
  ///
  /// By default, the value of [strokeWidth] is 2.0 pixels.
  final double strokeWidth;

  /// Defines how this [RefreshIndicator2] can be triggered when users overscroll.
  ///
  /// The [RefreshIndicator2] can be pulled out in two cases,
  ...
  ///
  /// If this is [RefreshIndicator2TriggerMode.anywhere], both of the cases above can be triggered.
  ///
  /// If this is [RefreshIndicator2TriggerMode.onEdge], only case 1 can be triggered.
  ///
  /// Defaults to [RefreshIndicator2TriggerMode.onEdge].
  final RefreshIndicator2TriggerMode triggerMode;
...
// The "value" of the circular progress indicator during a drag.
  ...
  @override
  void didUpdateWidget(covariant RefreshIndicator2 oldWidget) {
  ...
  bool _shouldStart(ScrollNotification notification) {
    ...
    return ((notification is ScrollStartNotification &&
            ...
            (notification is ScrollUpdateNotification &&
                notification.dragDetails != null &&
                widget.triggerMode == RefreshIndicator2TriggerMode.anywhere)) &&
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (_shouldStart(notification)) {
      setState(() {
        _mode = _RefreshIndicator2Mode.drag;
      });
      return false;
    }
    ...
    if (indicatorAtTopNow != _isIndicatorAtTop) {
      if (_mode == _RefreshIndicator2Mode.drag ||
          _mode == _RefreshIndicator2Mode.armed) {
        _dismiss(_RefreshIndicator2Mode.canceled);
      }
      ...
      if (_mode == _RefreshIndicator2Mode.armed &&
          notification.dragDetails == null) {
      ...
      switch (_mode) {
        case _RefreshIndicator2Mode.armed:
          _show();
        case _RefreshIndicator2Mode.drag:
          _dismiss(_RefreshIndicator2Mode.canceled);
        case _RefreshIndicator2Mode.canceled:
        case _RefreshIndicator2Mode.done:
        case _RefreshIndicator2Mode.refresh:
        case _RefreshIndicator2Mode.snap:
        case null:
          // do nothing
          break;
      }
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == _RefreshIndicator2Mode.drag ||
        _mode == _RefreshIndicator2Mode.armed);
    double newValue =
  ...
  Future<void> _dismiss(_RefreshIndicator2Mode newMode) async {
    await Future<void>.value();
    // This can only be called from _show() when refreshing and
    // _handleScrollNotification in response to a ScrollEndNotification or
    // direction change.
    assert(newMode == _RefreshIndicator2Mode.canceled ||
        newMode == _RefreshIndicator2Mode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode!) {
  ...
  void _show() {
    assert(_mode != _RefreshIndicator2Mode.refresh);
    assert(_mode != _RefreshIndicator2Mode.snap);
    final Completer<void> completer = Completer<void>();
    _pendingRefreshFuture = completer.future;
    _mode = _RefreshIndicator2Mode.snap;
    _positionController
        .animateTo(1.0 / _kDragSizeFactorLimit,
            duration: _kIndicatorSnapDuration)
        .then<void>((void value) {
      if (mounted && _mode == _RefreshIndicator2Mode.snap) {
        setState(() {
          // Show the indeterminate progress indicator.
          _mode = _RefreshIndicator2Mode.refresh;
        });

        final Future<void> refreshResult = widget.onRefresh();
        refreshResult.whenComplete(() {
          if (mounted && _mode == _RefreshIndicator2Mode.refresh) {
            completer.complete();
            _dismiss(_RefreshIndicator2Mode.done);
          }
        });
      }
    });
  ...
  ///
  /// Creating the [RefreshIndicator2] with a [GlobalKey<RefreshIndicator2State>]
  ...
  Future<void> show({bool atTop = true}) {
    if (_mode != _RefreshIndicator2Mode.refresh &&
        _mode != _RefreshIndicator2Mode.snap) {
      if (_mode == null) {
  ...
  Widget build(BuildContext context) {
    ...
    //     _mode == _RefreshIndicator2Mode.refresh ||
    //         _mode == _RefreshIndicator2Mode.done;
          ...
          Positioned(
            ...
            child: SizeTransition(
              ...
              child: Container(
                ...
                //                 .RefreshIndicator2SemanticLabel,
